	...
	/*******************************/
		...
		///////////////////////////	
			...
			for (int i = 0; i < cart_points.size(); i++)
			{
				...
				{
					poses[i].pose.position.y = 1;//(-2,-6,-2)
					cart_points[i].y = 1;
				}
				else
				{
	...
	while (nh.ok())
	{
		...
		if(coordinateCounter == -1)
		{
			...
			*/
			...
			vel_msg.linear.x += (rho*2/lookaheadRadius);
			/*
			ROS_INFO_STREAM(std::setprecision(2) << std::fixed
		...
		else
		{
			...
			*/
			...
			vel_msg.angular.z = 1.0*angleE;
			vel_msg.linear.x += (rho/lookaheadRadius);
			if(th == angleE)
			//if(angleE < 0.0001 && angleE > -0.0001) 
			//if(angleE == 0.00)
			{
				vel_msg.angular.z = 0;
			}
...
double computeAngleE(point point1, point robot_point, double currentTheta)
{
	/*
	ROS_INFO_STREAM(std::setprecision(2) << std::fixed
		<< "\natan inputs: 1) " << point1.y - robot_point.y 
		<< ", 2) " << point1.x - robot_point.x);
	
	ROS_INFO_STREAM(std::setprecision(2) << std::fixed
			<< "\natan result - currenttheta = " 
			<< (atan2((point1.y - robot_point.y), (point1.x - robot_point.x)) - currentTheta));
	*/
